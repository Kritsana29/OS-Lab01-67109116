...
def train_model_a():
    ...
    print("[Model A] Trying to acquire GPU 1...")

    acquired = gpu_1_lock.acquire(timeout=TIMEOUT)

    if not acquired:
        print(">> [OS WATCHDOG] Deadlock Detected on Model A!")
        print(">> [RECOVERY] Model A is releasing GPU 0...")
        gpu_0_lock.release()
        return

    print("[Model A] Successfully acquired GPU 1!")
